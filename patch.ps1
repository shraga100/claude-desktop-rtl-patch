...
function Restore-Patch {
    ...
    if ($ClaudeDir) {
        ...
        Write-Log "Restoring original files from validated backups..."
        $FilesToRestore = @(
        ...
        foreach ($Item in $FilesToRestore) {
            ...
            if (-not (Test-Path -LiteralPath $bak)) {
                Write-Warn "Backup for $(Split-Path $Item.Orig -Leaf) not found — skipped."
                continue
            }
            if (-not (Test-BackupUsable -Backup $bak)) {
                Write-Warn "Backup $(Split-Path $bak -Leaf) failed integrity check — skipped (file left as-is)."
                continue
            }
            Try {
                Wait-FileUnlock -Path $Item.Orig -TimeoutSeconds 15
                Copy-Item -LiteralPath $bak -Destination $Item.Orig -Force -ErrorAction Stop
    ...
    if ($IsRollback) {
        ...
        if ($Restored) { Write-Success "Restore complete: files reverted, Scheduled Task and certs removed." }
        else           { Write-Warn   "Restore finished. No file backups were applied (see warnings above), but Scheduled Task / certs / state were cleaned." }
    }
}
